...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = torch.where(self.alpha*x > x, self.alpha, 1.0)*dout
        # ========================
...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        x = self.grad_cache['x']
        dx = dout*(torch.exp(-x)/((1+torch.exp(-x))**2))
        # ========================
...
class TanH(Layer):
    ...
    def backward(self, dout):
        ...
        dx = dout*(1-((torch.exp(x)-torch.exp(-x))/(torch.exp(x)+torch.exp(-x)))**2)
        # ========================
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.empty((out_features, in_features)).normal_(0, wstd)
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        x_y = torch.gather(input=x, dim=1, index=y.reshape(-1, 1))
        loss = torch.mean((-1)*x_y + torch.log(torch.sum(torch.exp(x), dim=1)))
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        dx = torch.scatter(torch.zeros_like(x), 1, y.reshape(-1, 1), -1)
        dx += (torch.exp(x)/torch.sum(torch.exp(x), dim=1).reshape(-1,1))
        dx = (dx*dout)/N
        # ========================
